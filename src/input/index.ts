...
const pause = (map: InputMap, {state, edge}: {state: boolean, edge: boolean}) => {
...
const direction = (map: InputMap, oldDirection: Direction) => {
    let direction = 0;
    if (map.get('ArrowUp')) {
...
const walking = (map: InputMap, oldWalking: boolean) => {
    let sx = 0;
    let sy = 0;
    if (map.get('ArrowUp')) {
        sy += 1;

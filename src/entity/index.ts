...
// entity drawer).
export const entityHandler = (context: CanvasRenderingContext2D): EntityHandler => {
    ...
    return {
        ...
        spawn: (game, entity, initialState) => {
            ...
            const id = count;
            // destroy function of the entity (dereference it from the global
            // entity map for garbage collection)
            const destroy = () => map.delete(id);

            // instantiate the entity with its initial state and its destroy
            // function, put it in the map, and put its state in the game state.
